...
class HybridClient():
  ...
  def __init__( self, app_config: AppConfig, queue: IQueue, storage_model: Optional[ IStorageModel ], console: Console, playbook: Optional[ dict ] = None ):
    ...
    self._output_formatter = OutputFormatter( app_config=self._app_config )
  ...
  def _dispatch_response( self, response: JobResponse ):
    ...
    output = self._output_formatter.format( response.result )
  ...
  def run( self ):
    ...
    if self._app_config.job and self._playbook:
      ...
      if self._app_config.verbose:
        ...
        self.console.print( Markdown( self._output_formatter.format( object=new_job ) ) )
