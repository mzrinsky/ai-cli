...
"""
  ...
  def _init_queue(self) -> IQueue:
    return JobQueueFactory.create_queue(
  ...
  def _parse_args(self) -> SimpleNamespace:
    ...
    parser.add_argument( '-p', '--playbook', required=False, type=argparse.FileType( 'r' ), help='Playbook file to use' )
      ...
      if self._config.workflow_file:
        if self._config.verbose:
          if isinstance(self._config.workflow_file, TextIOWrapper):
            filename = self._config.workflow_file.name
          else:
